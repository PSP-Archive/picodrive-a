...
/* If on, MPU will call the interrupt_acknowledge callback when it services an
 * interrupt.
...
/* If on, MPU will actually halt when m68k_input_halt is called by the host.
...
/* Define the default mode the MPU runs in if none has been specified */
