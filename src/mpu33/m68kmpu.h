...
//#define CPU_PREF_ADDR	m68ki_mpu.pref_addr
//#define CPU_PREF_DATA	m68ki_mpu.pref_data
//#define CPU_ADD RESS_MASK m68ki_mpu.address_mask
//#define CPU_SR _MASK	m68ki_mpu.sr_mask
//#define CPU_INSTR_MODE	m68ki_mpu.instr_mode
//#define CPU_RUN_MODE	m68ki_mpu.run_mode
...
//#define m68ki_exception_cycle_table_000	 m68ki_mpu.cyc_exception
//#define CYC_BCC_NOTAKE_B m68ki_mpu.cyc_bcc_notake_b
//#define CYC_BCC_NOTAKE_W m68ki_mpu.cyc_bcc_notake_w
//#define CYC_DBCC_F_NOEXP m68ki_mpu.cyc_dbcc_f_noexp
//#define CYC_DBCC_F_EXP   m68ki_mpu.cyc_dbcc_f_exp
//#define CYC_SCC_R_FALSE  m68ki_mpu.cyc_scc_r_false
//#define CYC_MOVEM_W	   m68ki_mpu.cyc_movem_w
//#define CYC_MOVEM_L	   m68ki_mpu.cyc_movem_l
//#define CYC_SHIFT 	   m68ki_mpu.cyc_shift
//#define CYC_RESET 	   m68ki_mpu.cyc_reset
...
//	#define m68ki_tra ce_t1()
//	#define m68ki_tra ce_t0()
...
//#define ADD_CYC LES(A)	m68ki_remaining_cycles += (A)
//#define USE_CYC LES(A)	m68ki_remaining_cycles -= (A)
//#define SET_CYC LES(A)	m68ki_remaining_cycles = A
//#define GET_CYC LES() 	m68ki_remaining_cycles
//#define USE_ALL_CYC LES() m68ki_remaining_cycles = 0
...
typedef struct
{
//	uint cpu_dummy_type;	 /* MPU Type: 68000, 68010, 68EC020, or 68020 */
...
//INLINE void m68ki_stack _frame_0000(uint pc, uint sr, uint vector);
...
//INLINE void m68ki_exce ption_illegal(void);
...
//char* m68ki_disassemble_quick(unsigned int pc, unsigned int cpu _type);
...
{
	//m68ki_set_fc(fc); /* auto-disable (see m68kmpu.h) */
	//m68ki_check_address_error(address, MODE_READ, fc); /* auto-disable (see m68kmpu.h) */
...
{
	//m68ki_set_fc(fc); /* auto-disable (see m68kmpu.h) */
	//m68ki_check_address_error(address, MODE_WRITE, fc); /* auto-disable (see m68kmpu.h) */
...
//	uint od = 0;						/* Outer Displacement */
	...
	//if(CPU_ TYPE_IS_010_LESS(CPU_ TYPE))
	//{
		/* Calculate index */
 ...
 */
INLINE void m68ki_stack_ frame_0000(uint pc, uint sr, uint vector)
{
	/* Stack a 3-word frame if we are 68000 */
//	if(CPU_ TYPE == CPU_ TYPE_000)
//	{
...
//	m68ki_push_16(((!write)<<4) | ((!instruction)<<3) | m68ki_aerr_fc);
}
...
INLINE void m68ki_exception_trap(uint vector)
{
	...
	m68ki_stack_frame_3word(REG_PC, sr);//, vector);
...
{
	uint sr = m68ki_init_exception();
	m68ki_stack_frame_3word(REG_PC, sr);//, EXCEPTION_TRACE);
...
INLINE void m68ki_exception_privilege_violation(void)
{
	...
	m68ki_stack_frame_3word(REG_PPC, sr);//, EXCEPTION_PRIVILEGE_VIOLATION);
...
{
	uint sr = m68ki_init_exception();
	m68ki_stack_frame_3word(REG_PC, sr);//, EXCEPTION_FORMAT_ERROR);
...
void m68ki_exception_interrupt(uint int_level)
{
	...
	if(vector > 255)
	{
//		M68K_DEBUG_EMU((M68K_ LOG_FILEHANDLE "%s at %08x: Interrupt acknowledge returned invalid vector $%x\n",
//				 m68ki_mpu_names[CPU_ TYPE], ADDRESS_68K(REG_PC), vector));
		return;
	}
	...
	m68ki_stack_frame_3word(REG_PC, sr);//, vector);
//	if(FLAG_M && CPU_ TYPE_IS_EC020_PLUS(CPU_ TYPE))
//	{
...
//	}
	...
	if(vector<48)	CPU_INT_CYCLES += (m68ki_exception_cycle_table_000[vector]);
