...
static void m68k_op_reset(void)
{
    if(FLAG_S)
    {
        ...
        m68ki_remaining_cycles -=(CYC_RESET);
        return;
    }
    m68ki_exception_privilege_violation();
}
...
static void m68k_op_ror_8_r(void)
{
    ...
    if(orig_shift != 0)
    {
        m68ki_remaining_cycles -=(orig_shift<<CYC_SHIFT);
...
static void m68k_op_roxr_8_r(void)
{
    ...
    /*  while(9<shift){ shift-=9;   };*/
        ...
        if(9<shift){shift-=9;   }}}}}}};
#endif
        src   = MASK_OUT_ABOVE_8(*r_dst);
        res   = ROR_9(src | (XFLAG_AS_1() << 8), shift);
...
static void m68k_op_roxr_16_r(void)
{
    ...
    /*  while(17<shift){    shift-=17;  };*/
        ...
        res   = ROR_17(src | (XFLAG_AS_1() << 16), shift);
...
#endif
    src = *r_dst;
    res = MASK_OUT_ABOVE_32((ROR_33(src, shift) & ~(1 << (32 - shift))) | (XFLAG_AS_1() << (32 - shift)));
...
#endif
        src   = MASK_OUT_ABOVE_8(*r_dst);
        res   = ROL_9(src | (XFLAG_AS_1() << 8), shift);
...
#endif
        src   = MASK_OUT_ABOVE_16(*r_dst);
        res   = ROL_17(src | (XFLAG_AS_1() << 16), shift);
...
#endif
    src = *r_dst;
    res = MASK_OUT_ABOVE_32((ROL_33(src, shift) & ~(1 << (shift - 1))) | (XFLAG_AS_1() << (shift - 1)));
