...
//  m68ki_stack_frame_0000(REG_PPC, sr, EXCEPTION_1010);
    ...
    USE_CYCLES(4/*CYC_EXCEPTION[EXCEPTION_1010]*/ - m68ki_cycles[REG_IR]);
}
...
//  m68ki_stack_frame_0000(REG_PPC, sr, EXCEPTION_1111);
    ...
    /* Use up some clock cycles and undo the instruction's cycles */
    USE_CYCLES(4/*CYC_EXCEPTION[EXCEPTION_1111]*/ - m68ki_cycles[REG_IR]);
}
...
M68KMAKE_OP(cmpi, 32, ., d)
{
    ...
    //m68ki_cmpild_callback(src, REG_IR & 7);        /* auto-disable (see m68kmpu.h) */
...
M68KMAKE_OP(dbf, 16, ., .)
{
    ...
    *r_dst = MASK_OUT_BELOW_16(*r_dst) | res;
    if(res != 0xffff)
    {
        ...
        REG_PC -= 2;
        m68ki_branch_16(offset);
        //USE_CYCLES(2);//test 12+2 14
        //USE_CYCLES(CYC_DBCC_F_NOEXP);//original 12-2 ==10
        return;
    }
    REG_PC += 2;
    //USE_CYCLES(-2);//test 12-2 10
    //USE_CYCLES(CYC_DBCC_F_EXP);//original 12+2==14
//えーとdbfはどおだっけ？？？
}
...
M68KMAKE_OP(reset, 0, ., .)
{
    if(FLAG_S)
    {
        //m68ki_output_reset();          /* auto-disable (see m68kmpu.h) */
...
M68KMAKE_OP(rte, 32, ., .)
{
    if(FLAG_S)
    {
        ...
        //m68ki_rte_callback();          /* auto-disable (see m68kmpu.h) */
...
M68KMAKE_OP(trap, 0, ., .)
{
    ...
    m68ki_exception_trap(EXCEPTION_TRAP_BASE + (REG_IR & 0xf));    /* HJB 990403 */
}
